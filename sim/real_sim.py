...
class ContextualBanditSimulator(object):
    ...
    def __init__(self, policy_list:List[BaseContextualPolicy], bandit:BaseBandit, n_sims:int, n_steps:int, n_arms:int, n_features: int, data_type) -> None:
        """クラスの初期化
        Args:
    ...
    def get_exp_reward(self, reward, step, opt_actions):
        if self.data_type == "mushroom":
            # print("sim/real_sim.py/get_exp_reward mushroom")
            if reward == -35:
                #絶対に毒キノコ
                exp = -15
            elif reward == 0:
            ...
            # print("sim/real_sim.py/get_exp_reward else")
            return reward
